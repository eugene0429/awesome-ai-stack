...
| XiuRouter | Usage-based model API service for developers — one service for GPT, Claude, Gemini, and other listed models through OpenAI Chat Completions/Responses, Anthropic Messages, and Gemini GenerateContent routes, with scoped API keys, a public pricing catalog, playground, and usage records. Model availability, pricing, and protocol support vary by the live catalog and target request. | [router.xiu.ai](https://router.xiu.ai/) | `SaaS` |
